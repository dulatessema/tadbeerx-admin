...
export default function LanguagesPage() {
  ...
  const openModal = (mode: 'create' | 'edit', item?: Language) => {
  ...
  const handleToggleActive = async (id: string) => {
    try {
      const currentItem = languages.find(item => item.id === id);
      if (currentItem) {
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">Manage worker languages</p>
          </div>
          <button
...
function LanguageModal({ data, onClose, onSave }: LanguageModalProps) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        <h3 className="text-lg font-medium text-gray-900 mb-4">
          {data.mode === 'create' ? 'Add' : 'Edit'} Language
        </h3>
